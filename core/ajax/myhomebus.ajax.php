...
try {
	...
	if (init('action') == 'checktemplate') {
		$myhome = myhomebus::byLogicalId(init('id'), 'myhomebus');
        if (is_object($myhome)) {
	...
	if (init('action') == 'AppliTemplate') {
		...
		if (is_object($EqLogic)){
			ajax::success($EqLogic->applyModuleConfiguration(init('template')));
		}else{
			ajax::success(false);
		}		
	}
	...
	if (init('action') == 'updateMemory') {
        $mem = myhomebus::byId(init('id'));
        if (!is_object($mem)) {
            throw new Exception(__('Equipement inconnu verifié l\'id', __FILE__));
        }
		myhomebus::deleteMemory(init('idtrame'));
		//sleep(5);
		myhomebus::checkMemory(init('idtrame'),2);
